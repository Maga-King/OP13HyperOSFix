...
import local.mio.op13hyperosfix.DiagnosticMarkers;
...
public final class HookEntry implements IXposedHookLoadPackage, IXposedHookZygoteInit {
    ...
    private static final String NOTES_APP = "com.miui.notes";
    private static final String NOTES_ALL_FINISH_THREAD = "todoAniWorker";
    ...
    private static final int EFFECT_TODO_FINISH_ONE = 206;
    private static final int EFFECT_TODO_FINISH_ALL = 205;
    ...
    private static final int VIBRATION_USAGE_CLASS_MASK = 0x0f;
    private static final int VIBRATION_USAGE_CLASS_FEEDBACK = 0x02;
    ...
    private static final AtomicBoolean NOTES_ALL_FINISH_LOGGED = new AtomicBoolean(false);
    ...
    private static final Set<Integer> LOGGED_NATIVE_STRENGTHS =
            ConcurrentHashMap.newKeySet();
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        if (SYSTEM_FRAMEWORK.equals(lpparam.packageName)) {
            ...
            installHook("framework-feedback-strength-bridge",
                    () -> hookFrameworkFeedbackStrength(lpparam.classLoader));
        ...
        if (SETTINGS_APP.equals(lpparam.packageName)) {
            ...
            DiagnosticMarkers.installApplicationReporter("haptics_settings");
            return;
        }

        if (NOTES_APP.equals(lpparam.packageName)) {
            installHook("notes-all-finished-haptic",
                    () -> hookNotesAllFinishedHaptic(lpparam.classLoader));
            return;
        }
        ...
        DiagnosticMarkers.installApplicationReporter("haptics_systemui");
    ...
    /**
     * OS4 Notes regressed both completion paths to effect 206. The final-item
     * animation still runs on its dedicated todoAniWorker, so restore the
     * original Xiaomi split without depending on the app's obfuscated classes:
     * main-thread completion stays 206 while all-done uses 205.
     */
    private static void hookNotesAllFinishedHaptic(ClassLoader classLoader) {
        Class<?> hapticClass = XposedHelpers.findClass(
                "miui.util.HapticFeedbackUtil", classLoader);
        XposedHelpers.findAndHookMethod(
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        if ((int) param.args[0] != EFFECT_TODO_FINISH_ONE
                                || !NOTES_ALL_FINISH_THREAD.equals(
                                        Thread.currentThread().getName())) {
                            return;
                        }
                        param.args[0] = EFFECT_TODO_FINISH_ALL;
                        if (NOTES_ALL_FINISH_LOGGED.compareAndSet(false, true)) {
                            XposedBridge.log(TAG + ": Notes all-finished haptic "
                                    + EFFECT_TODO_FINISH_ONE + " -> "
                                    + EFFECT_TODO_FINISH_ALL);
                        }
                    }
                });
        XposedBridge.log(TAG + ": Notes completion haptic split active");
    }
        ...
        XposedHelpers.findAndHookMethod(
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        ...
                        PENDING_AUDIO_RTP.remove();
                        boolean matchingRoute = route != null
                        ...
                        if (matchingRoute) {
                            ...
                            if (route.assetEffectId == 192) {
                                param.setResult(0L);
                                XposedBridge.log(TAG
                                        + ": video RTP 192 unavailable; suppress effect 59 fallback");
                                return;
                            }
                        ...
                        int effectId = (int) (long) param.args[0];
                        int rawStrength = prepareNativeOnePlusPrebaked(
                                param.thisObject, effectId);
                        if (rawStrength == 0) {
                            scheduleNativeCompletion(
                                    param.thisObject,
                                    1L,
                            ...
                            // ColorOS always performs the waveform at STRONG after
                            // encoding the real 800..2400 strength via setAmplitude.
                            param.args[1] = (long) STRENGTH_STRONG;
                        }
                    }
                });
            ...
            XposedHelpers.findAndHookMethod(
                    ...
                    new XC_MethodHook() {
                        ...
                        protected void beforeHookedMethod(MethodHookParam param) {
                            ...
                            } else if (shouldRoutePackagedRtp(source)) {
                            ...
                            if (route != null) {
                                ...
                                if (duration > 0L) {
                                    captureDefaultHalCompletionTarget(param.thisObject);
                                    scheduleNativeCompletion(
                                            param.thisObject,
                                            duration,
                                            (long) param.args[0],
                            ...
                            if (rawStrength == 0) {
                                ...
                                XposedHelpers.setIntField(
                                        segment, "mEffectStrength", STRENGTH_STRONG);
                            }
                        }
                    });
            XposedBridge.hookAllMethods(
    ...
    private static void captureDefaultHalCompletionTarget(Object defaultHal) {
        Object callbacks = XposedHelpers.getObjectField(defaultHal, "mCallbacks");
        if (callbacks == null) return;
        int vibratorId = XposedHelpers.getIntField(defaultHal, "mVibratorId");
        NATIVE_COMPLETION_TARGETS.put(
                defaultHal,
                new NativeCompletionTarget(
                        vibratorId, callbacks, "onVibrationStepComplete"));
    }

    /**
     * ColorOS's qcom linear-motor path encodes raw strength through the base
     * IVibrator setAmplitude transaction, then performs the prebaked waveform
     * with effect strength STRONG. Return -1 when this is not an Oplus HAL, 0
     * when the request should be silent, or the applied raw value.
     */
    private static int prepareNativeOnePlusPrebaked(Object transport, int effectId) {
        if (!RtpPlayer.hasOnePlusTransport()) return -1;

        int rawStrength = RtpPlayer.consumeNativePrebakedStrength(effectId);
        if (rawStrength <= 0) return 0;
        if (!RtpPlayer.setNativePrebakedStrength(rawStrength)) {
            return -1;
        }
        if (LOGGED_NATIVE_STRENGTHS.add(effectId)) {
            XposedBridge.log(TAG + ": native Oplus strength effect=" + effectId
                    + " raw=" + rawStrength);
        }
        return rawStrength;
    }
    ...
    private static boolean shouldRoutePackagedRtp(int effectId) {
        // Keep every AOSP predefined effect on OnePlus's calibrated native path.
        // Only Xiaomi private IDs with an explicit packaged asset use raw RTP.
        return RtpPlayer.hasEffect(effectId) && effectId > 12;
    }
    ...
    /** Continuously scale non-prebaked feedback (Step, Primitive and PWLE). */
    private static void hookFrameworkFeedbackStrength(ClassLoader classLoader) {
        Class<?> scalerClass = XposedHelpers.findClass(
                "com.android.server.vibrator.VibrationScaler", classLoader);
        XposedHelpers.findAndHookMethod(
                scalerClass,
                "getScaleFactor",
                int.class,
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        if ((usage & VIBRATION_USAGE_CLASS_MASK)
                                != VIBRATION_USAGE_CLASS_FEEDBACK) {
                            return;
                        }

                        param.setResult(RtpPlayer.currentMotorScale());
                    }
                });
        XposedBridge.log(TAG
                + ": continuous Step/Primitive/PWLE feedback scaling active");
    }
    ...
    private static void hookSettingsHapticVideos(ClassLoader classLoader) {
        ...
        XposedHelpers.findAndHookMethod(
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        int effectId = (int) param.args[0];
                        // OS4 selects the dual-motor SRT on this port and asks
                        // for effect 1. The OnePlus 13 has one actuator; use the
                        // matching Xiaomi single-motor todo-completion cue.
                        if (effectId == 1) {
                            effectId = 171;
                            param.args[0] = effectId;
                        }
    ...
    private static int mapXiaomiEffect(int source) {
        switch (source) {
            case 7: return 2;    // keyboard: calibrated OnePlus strong click
            ...
            case 204: return 363;
            case 205: return 362;
            case 206: return 46;  // native fallback when the Xiaomi RTP cannot start
            case 207: return 12;
            case 208: return 106;
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        if ((long) param.args[0] != 24L
                                || !stackContainsFodFailureRunnable()) {
                            return;
                        }
    ...
    private static boolean stackContainsFodFailureRunnable() {
        for (StackTraceElement element : Thread.currentThread().getStackTrace()) {
            String className = element.getClassName();
            if ("run".equals(element.getMethodName())
                    && className.startsWith("com.miui.keyguard.biometrics.fod.")
                    && className.contains("MiuiGxzwAnimView")
