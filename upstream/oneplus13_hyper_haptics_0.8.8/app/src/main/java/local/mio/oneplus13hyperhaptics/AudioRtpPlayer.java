...
final class AudioRtpPlayer {
    ...
    static boolean performVideo(Context context, int effectId) {
        ...
        try {
            Context moduleContext = context.createPackageContext(
                    "local.mio.oneplus13hyperhaptics",
                    Context.CONTEXT_IGNORE_SECURITY);
            int rawId = moduleContext.getResources().getIdentifier(
                    "haptic_video_192", "raw", moduleContext.getPackageName());
            if (rawId == 0) return false;
            AssetFileDescriptor mediaFile =
                    moduleContext.getResources().openRawResourceFd(rawId);
    ...
    static float readOfficialStrength(Context context) {
        ...
        try {
            // This module expands OS4's visual 0..100 seekbar to its service's
            // native 0..1.5 storage range. Normalize it back to a transport
            // gain so Settings-local haptic-A/root paths obey the same control.
            float stored = Settings.System.getFloat(
                    context.getContentResolver(),
                    "haptic_feedback_infinite_intensity",
                    1.5f);
            return Math.max(0.0f, Math.min(1.0f, stored / 1.5f));
